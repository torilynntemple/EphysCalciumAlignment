...
%% Parameters
dt = median(diff(ms.ms.time))/1000; % Conversion from ms to s
Fs = 1/dt;
z_threshold = 2;

[bFilt,aFilt] = butter(2,  2/(30/2), 'low');

for trace_i = 1:ms.ms.numNeurons;
raw_trace = ms.ms.RawTraces(:,trace_i);
...
ms.ms.Binary(:,trace_i) = binary_trace;
end
