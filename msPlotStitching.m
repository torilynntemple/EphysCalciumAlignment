...
function msPlotStitching(ms, cellNum, LFP_timestamp, startTimes)
    ...
    for i =1:length(ms)        
        if any(ismember(fields(ms(i).ms),'RawTraces'))
            ...
            trace_overlap(ms(i).ms, cellNum, LFP_timestamp, startTimes(1,i), ms(i).ms.trace); % plotting REM  from the calcium imaging data
            xlabel('Frames');
            ylabel('Amplitude');
            hold on   
